...
const { selectUseNamespaces, parseXMLNode } = require('oniyi-utils-xml');

// internal modules
const { toInteger, extractIdFromURN } = require('../utils/utility');
...
const textValueSelectors = {
  ...
  versionLabel: {
    selector: 'string(td:versionLabel/text())',
    transform: toInteger,
  },
  totalMediaSize: {
    selector: 'string(td:totalMediaSize/text())',
    transform: toInteger,
  },
  title: 'string(atom:title[@type="text"]/text())',
  summary: 'string(atom:summary[@type="text"]/text())',
  added: 'string(td:added/text())',
  created: 'string(td:created/text())',
  published: 'string(atom:published/text())',
  updated: 'string(atom:updated/text())',
  contentUpdated: 'string(td:contentUpdated/text())',
  modified: 'string(td:modified/text())',
...
// since addedBy, author and modifier share the same properties,
// this function can be reused by providing different selector
...
const parsePolicy = node => parseXMLNode(node, {
